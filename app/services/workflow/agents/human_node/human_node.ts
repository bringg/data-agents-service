...
export const humanNode = async (state: SuperWorkflowStateType) => {
	const { content } = await SuperWorkflow.llm.invoke(
		`A customer is going to read your output. Transpile the given instructions onto a first-person conversation. Reply only with what the customer needs to read. instructions: ${state.instructions}`
	);
